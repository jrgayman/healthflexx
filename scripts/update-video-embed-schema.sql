...
-- Add updated type constraint to include video-embed
ALTER TABLE posts
...
CHECK (
  ...
  (type = 'video-embed' AND video_url IS NOT NULL AND embed_code IS NOT NULL AND app_store_url IS NULL AND play_store_url IS NULL AND web_url IS NULL) OR
