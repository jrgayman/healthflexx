...
export default function ActivityManagement() {
  ...
  async function fetchData() {
    try {
      // Fetch patients with activity goals
    ...
    if (statusFilter) {
      filtered = filtered.filter(patient => {
        if (statusFilter === 'active') {
          return patient.activity_goals?.some(goal => goal.active);
        }
        if (statusFilter === 'no_goal') {
          return !patient.activity_goals?.length;
        }
        return true;
      });
    }
      ...
      <div className="mb-8 grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <FilterDropdown
          ...
          options={[
            ...
            { value: 'no_goal', label: 'No Goals Set' }
          ]}
        />
      </div>
