...
export default function TypeForm({ type = null, classifications, onSave, onClose }) {
  ...
  const [imagePreview, setImagePreview] = useState(type?.image_url);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let imageUrl = type?.image_url;

      // Handle image upload/update
      if (imageFile) {
        // Delete old image if exists
        if (type?.image_url) {
          await deleteDeviceImage(type.image_url);
        }
        // Upload new image
      ...
      toast.success(type ? 'Model updated!' : 'Model added!');
      onClose();
    } catch (error) {
