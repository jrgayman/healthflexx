  ...
  async function fetchReadings() {
    try {
      const query = supabase
        ...
        .select('*, reading_types!inner(id, name, code, unit, value_type, icon, normal_min, normal_max, critical_low, critical_high)')
        .eq('user_id', patient.id)
        .order('reading_date', { ascending: false });

      if (readingTypeFilter) {
        query.eq('reading_type_id', readingTypeFilter);
      }
      ...
      if (startDate) {
        query.gte('reading_date', startDate.toISOString());
      }
